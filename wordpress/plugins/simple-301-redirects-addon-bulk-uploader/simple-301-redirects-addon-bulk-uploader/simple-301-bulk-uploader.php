...
if (!class_exists("Bulk301Uploader")) {
	class Bulk301Uploader {
			 ...
			 * @var string
			 */
			private $redirects_option = 's301r_redirects'; // This was originally '301_redirects'
            ...
            */
            function save_bulk_redirects($data, $auto_detect = 0)
            {
                ...
                $current_redirects = get_option($this->redirects_option);
                    ...
                    if (($handle = fopen($data["tmp_name"], "r")) !== FALSE) {
                        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
                            ...
                            if (!isset($current_redirects[$data[0]]) && $data[1] !== '') {
                                ...
                                // Change to use same format as simple-301-redirects plugin
								$current_redirects[] = [
									'request' => $data[0],
									'destination' => $data[1],
								];
                        ...
                        fclose($handle);
                        update_option($this->redirects_option, $current_redirects);
                    }
                    }
                  }
                else
            ...
            */
            ...
            function clear_301_redirects() {
                update_option($this->redirects_option, '');
