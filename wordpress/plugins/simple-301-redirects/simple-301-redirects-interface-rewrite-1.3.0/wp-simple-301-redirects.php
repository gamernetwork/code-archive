	...
	class Simple301Redirects {
			...
			<div class="wrap s301r">
				...
				<?php elseif ( isset( $_GET['s301r_action'] ) && 'edit' === sanitize_text_field( $_GET['s301r_action'] ) && isset( $_GET['index'] ) && isset( $_GET['hash'] ) ) : ?>
					...
					<?php else : ?>
						<form method="post" id="simple_301_redirects_form" action="<?php echo esc_url( $this->settings_url . '&s301r_action=edit' ); ?>">
							...
							<input type="hidden" name="hash" value="<?php echo esc_attr( $hash ); ?>" />
							<p>
								<?php esc_html_e( 'Request', 'simple-301-redirects' ); ?><br/>
								<input type="text" name="request" value="<?php echo esc_attr( $redirect['request'] ); ?>" style="min-width: 600px"/>

								<br/>

								<?php esc_html_e( 'Destination', 'simple-301-redirects' ); ?><br/>
								<input type="text" name="destination" value="<?php echo esc_attr( $redirect['destination'] ); ?>" style="min-width: 600px"/>

								<div class="redirect-options" style="margin-top: 1rem;">
				...
				<?php else : ?>
					<form method="post" id="simple_301_redirects_form" action="<?php echo esc_url( $this->settings_url . '&s301r_action=add' ); ?>">
						...
						<p>
							<?php esc_html_e( 'Request', 'simple-301-redirects' ); ?><br/>
							<input type="text" name="request" style="min-width: 600px"/>

							<br/>

							<?php esc_html_e( 'Destination', 'simple-301-redirects' ); ?><br/>
							<input type="text" name="destination" style="min-width: 600px"/>
		...
		function list_redirects() {
			...
			if ( ! empty( $redirects ) && is_array( $redirects ) ) {
				...
				// Show newest redirects first
				$redirects = array_reverse($redirects);

				// Only show the latest 250 so the render doesn't take forever
				$redirects = array_slice($redirects, 0, 250);
